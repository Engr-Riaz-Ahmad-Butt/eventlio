...
let AuthService = AuthService_1 = class AuthService {
    ...
    async register(dto) {
        ...
        const role = dto.role === client_1.Role.VENDOR_OWNER ? client_1.Role.VENDOR_OWNER : client_1.Role.CLIENT;
    ...
    async login(dto) {
        ...
        if (!user.isEmailVerified) {
            ...
            this.logger.log(`OTP for ${dto.email}: ${otp}`);
            throw new common_1.UnauthorizedException('Email not verified. A new OTP has been sent.');
        }
        const tokens = await this.generateTokens(user.id, user.email, user.role);
        return {
